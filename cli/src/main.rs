...
enum Command {
    ...
    /// Print a method document — the reasoning peira checks for — version-stamped
    /// so a block regenerated from it records which peira it came from.
    Method {
        /// A method name; omit to list them.
        name: Option<String>,
    },
}
...
/// The provenance line stamped ahead of an emitted method document, so a block
/// regenerated from it records the peira version it came from and how to refresh it.
fn method_header(name: &str) -> String {
    format!(
        "<!-- generated by peira v{} from docs/method/{name}.md · regenerate: peira method {name} -->",
        env!("CARGO_PKG_VERSION")
    )
}

fn cmd_method(name: Option<String>) -> Result<u8, String> {
    let Some(name) = name else {
        println!("method documents (peira v{}):\n", env!("CARGO_PKG_VERSION"));
        for (n, _) in METHODS {
            println!("  {n}");
        }
...
fn run() -> Result<u8, String> {
    match Cli::parse().command {
        ...
        Command::Method { name } => cmd_method(name),
    }
}
...
mod tests {
    ...
    /// peira must expose the anti-summarization method so a skill block can be
    /// regenerated from the binary rather than hand-copied. Asserts the doors are all
    /// present, so removing one from the doc is caught here.
    #[test]
    fn method_exposes_the_anti_summarization_doors() {
