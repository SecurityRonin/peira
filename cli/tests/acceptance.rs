...
use elenchus_core::{load, Graph, NodeId};
...
fn control_a_the_overclaim_is_blocked_by_at_least_five_lenses() {
    ...
    let expected = [
        "ELEN-CRITERION-UNDECLARED",  // 立極 — judged with no declared standard
        "ELEN-FUNCTION-AS-SUBSTANCE", // 體用 — what it did, stated as what it is
        "ELEN-CLASS-EXTENSION-UNDECLARED", // 白馬非馬 — one token, quantified universally
        "ELEN-CORNERS-UNADDRESSED",   // 四句 — contested, binarised
        "ELEN-WARRANT-MISSING",       // Toulmin — the unwritten rule
        "ELEN-CAUSAL-RUNG-UNREACHED", // Pearl — rung 3 from rung 1 data
        "ELEN-BOUNDARIES-MISSING",    // no conditions under which it would change
        "ELEN-LINT-FORBIDDEN-VERB",   // "proves"
    ];
...
fn control_b_freezes_a_packet_carrying_the_three_moments() {
    let graph = vault("bounded");
    let packet = elenchus_court::freeze(&graph, &NodeId::new("c-bounded"))
...
fn a_frozen_packet_stops_verifying_when_its_evidence_changes() {
    let graph = vault("bounded");
    let packet = elenchus_court::freeze(&graph, &NodeId::new("c-bounded")).unwrap();
    assert!(elenchus_court::verify(&graph, &packet));

    // Re-load and mutate an observation the packet cites.
    let mut tampered = vault("bounded");
    let doctored = elenchus_core::parse_node(
...
pramana: perception\nsupports: [\"c-bounded grade=G2 by=albert via=perception\"]\n---\n",
    ...
    assert!(
        !elenchus_court::verify(&tampered, &packet),
