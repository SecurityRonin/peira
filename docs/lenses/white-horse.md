...
sliding between a type and its tokens, or between intension and extension

## What it does

a claim quantifying over a class must declare that class's extension

## Enforced gates

These block a citation packet deterministically — no model in the loop:

- `PEIR-CLASS-EXTENSION-UNDECLARED`

## Worked example

One white horse was examined, and the claim is made of horses at large. 公孫龍's 白馬非馬 turns on exactly this: "white horse" and "horse" have different extensions, so what holds of the qualified subclass need not hold of the class, however natural the plural feels.
